  ...
  try {
    ...
    if (existingMentor && existingMentor.verificationStatus === "REJECTED") {
      ...
      if (Array.isArray(existingMentor.documents)) {
        for (const doc of existingMentor.documents as Array<{ publicId?: string }>) {
      ...
      if (oldPublicIds.length > 0) {
        await Promise.all(
          oldPublicIds.map((id) => cloudinary.uploader.destroy(id).catch(() => null))
        );
      }
    ...
    throw new AppError(httpStatus.BAD_GATEWAY, "Application Failed. Try Again")
  }
};
